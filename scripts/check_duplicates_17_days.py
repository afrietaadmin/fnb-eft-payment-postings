...
logger = setup_logging('check_duplicates_17_days')
app = create_app()

def check_duplicate_payment(txn, days_back=17):
    """Check if this CID already has a posted payment with the same amount in the last N days"""
    ...
    try:
        ...
        # Find other posted payments for this CID with SAME AMOUNT in last N days
        duplicate = Transaction.query.filter(
            Transaction.CID == txn.CID,
            Transaction.amount == txn.amount,  # Same amount
...
def main():
    with app.app_context():
        try:
            ...
            print("\n" + "="*80)
            print(f"DUPLICATE CHECK - LAST {DAYS_BACK} DAYS (SAME CID + SAME AMOUNT)")
            print("="*80 + "\n")
            ...
            print(f"Duplicate detection window: {DAYS_BACK} days")
            print(f"Duplicate criteria: Same CID + Same Amount\n")
            ...
            for txn in new_transactions:
                ...
                duplicate = check_duplicate_payment(txn, days_back=DAYS_BACK)
            ...
            print("\n" + "="*80)
            print(f"⚠️  DUPLICATES DETECTED (Same CID + Same Amount within last {DAYS_BACK} days)")
            print("="*80 + "\n")
            ...
            if duplicates_found:
                ...
                for item in duplicates_found:
                    ...
                    print(f"     Existing Posted:  {dup.entryId} | R{dup.amount:.2f} | Posted: {dup.postedDate.strftime('%Y-%m-%d')} ({days_ago} days ago)")
                    print(f"     Match:            Same CID ({txn.CID}) + Same Amount (R{txn.amount:.2f})")
                    print()
                    total_duplicate_amount += txn.amount

                print(f"  Total Duplicates: {len(duplicates_found)} transactions, R{total_duplicate_amount:.2f}")
            else:
                print("  (None)")
            ...
            print(f"  Detection Window:    Last {DAYS_BACK} days (Same CID + Same Amount)")
            print("="*80 + "\n")
            ...
            if duplicates_found:
                ...
                for item in duplicates_found:
                    ...
                    print(f"  - CID{txn.CID}: R{txn.amount:.2f} paid {days_ago} days ago (Entry: {dup.entryId})")
                print()
            else:
                print(f"✅ No duplicates found - all transactions are safe to post!\n")

            logger.info(f'Duplicate check (17 days, same amount) complete: {len(safe_to_post)} safe, {len(duplicates_found)} duplicates, {len(no_cid)} no CID')
