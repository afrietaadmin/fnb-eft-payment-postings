...
logger = setup_logging('check_duplicates_15_days')
app = create_app()

def check_duplicate_payment(txn, days_back=15):
    """Check if this CID already has a posted payment in the last N days"""
...
def main():
    with app.app_context():
        try:
            ...
            print("\n" + "="*80)
            print(f"DUPLICATE CHECK - LAST {DAYS_BACK} DAYS (NO POSTING TO UISP)")
            print("="*80 + "\n")
            ...
            print("\n" + "="*80)
            print(f"⚠️  DUPLICATES DETECTED (Already posted within last {DAYS_BACK} days)")
            print("="*80 + "\n")
            ...
            print(f"  Detection Window:    Last {DAYS_BACK} days")
            print("="*80 + "\n")
            ...
            if duplicates_found:
                ...
                for item in duplicates_found:
                    ...
                    print(f"  - CID{txn.CID}: Last payment {days_ago} days ago (R{dup.amount:.2f})")
                print()
            else:
                print(f"✅ No duplicates found - all transactions are safe to post!\n")

            logger.info(f'Duplicate check (15 days) complete: {len(safe_to_post)} safe, {len(duplicates_found)} duplicates, {len(no_cid)} no CID')
